...
- `crates/cli` (`dop`) — clap-derive CLI; `src/cli.rs` holds the `Cli` parser struct. Design rule: **mechanism in core, policy in frontends** — the timeout and signal handling are CLI policy. After acquiring the guard, `main` blocks on an mpsc channel: `recv_timeout(--timeout-ms)` or `recv()` forever; a `ctrlc` handler (with the `termination` feature: SIGINT/SIGTERM/SIGHUP) sends `()` to end the wait. Every exit path is "main returns normally so the guard drops" — never `process::exit`, which skips `Drop`. Core never blocks.
